...
#     sample2 = {}
    ...
    # Calling script to generate sequence logos
    subprocess.check_call('sudo python3 {} {} {}'.format(os.path.join('app','seqlogo.py'), taskId, data_mount), shell=True)
    seqlogos = getSeqLogosImages(sample_data)
    # seqlogos = {}

    # Calling script to generate gibbsclusters
    subprocess.check_call('sudo python3 {} {} {}'.format(os.path.join('app', 'gibbscluster.py'), taskId, data_mount), shell=True)
...
def createGibbsBar():
    ...
    if len(barLocation) ==1:
        ...
        subprocess.check_call('sudo python3 {} {} {} {} {}'.format(os.path.join('app', 'gibbsclusterBarGraph.py'), taskId, data_mount, sample, replicate), shell=True)
    ...
    if len(cluster) == 0:
        ...
        if len(seqClusters) != int(cluster):
            subprocess.check_call('sudo python3 {} {} {} {} {} {}'.format(os.path.join('app', 'gibbsclusterSeqLogo.py'), taskId, data_mount, sample, replicate, int(cluster)), shell=True)
            seqClusters = [[x[4:], "Could not be calculated"] for x in sorted(glob.glob(f'app/static/images/{taskId}/{sample}/gibbscluster/{replicate}/logos/gibbs_logos_*of{cluster}*.jpg'))]
...
def getSeqLogo():
    ...
    subprocess.check_call('sudo python3 {} {} {} {} {} {}'.format(os.path.join('app','seqlogoAPI.py'),peptides_location_forseqlogo,seqLogoLocation,name,nine_mers,total_peptides), shell=True)
