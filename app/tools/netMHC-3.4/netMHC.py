...
os.environ["NMHOME"] = "/mnt/c/Users/ASUS/Documents/sem3/thesis/firstdemo/app/tools/netMHC-3.4"
...
def readseqs(filename):
    ...
    entries, order = sequences.readfsa_mn(file)
    file.close()
...
def predict(NN, infilename, tdir, pdir, platform, mhc, outfilename, plen, wl, newpepfilename):
...
def printresults(NN, entries, order, preds, allele, methodlist, plen, www):
    thr1 = 50
    thr2 = 500
    ...
    if www:    
        ...
        downloadstring = ''
    xlsdict       = {}
    webheader         = "%4s %10s %13s %12s %10s %15s %10s" % ('pos', 'peptide', 'logscore', 'affinity(nM)', 'Bind Level', 'Protein Name', 'Allele')
    ...
    for entry in order:
        ...
        if NN:
			...
			print score
			sys.exit()
                print "%4i %10s %13.3f %12d %10s %15s %10s" % (int(pos), pep, float(score), int(aff), bindlevel, name, allele)
        xlsdict[entry].sort()
        print "--------------------------------------------------------------------------------------------------"
    print downloadstring
    return(xlsdict)
...
def main():
    ...
    methodlist = {}
    dummy, predtempname = tempfile.mkstemp(dir=tmpdir)
    for allele in alleles:
        ...
        for entry in seqs:
            ...
            allele2, direct = predict(NNpred, pepfilename, tmpdir, preddir, platform, allele, predfilename, peplen, whitelist, predtempname )
	    ave = False
	    if (not peplen == 9) and direct and useapprox:
		    direct = False
		    ave    = True
		    mypredfilename = predfilename + "_2"
		    dummy1, dummy2 = predict(NNpred, pepfilename, tmpdir, preddir, platform, allele, mypredfilename, peplen, {}, predtempname)
