...
if args.mode == 'prediction':
    ###check input
    [HLAlist, namepeptides, peptidelength, fastafile, thresholdvalueadjust, resd] = sware_h_checkargs.checkarg(args)

    if peptidelength == None:
        ###generate features
        featurecombine = sware_f_combinefeature.combfea(str(args.length), HLAlist, namepeptides)
        ###use model
        sware_d_prediction.pred(featurecombine, str(args.length), fastafile, thresholdvalueadjust, res=resd)
    else:
        ###generate features
        featurecombine = sware_f_combinefeature.combfea(str(peptidelength), HLAlist, namepeptides)        
        ###wirte in arff and predict
        sware_d_prediction.pred(featurecombine, str(peptidelength), fastafile, thresholdvalueadjust, res=resd)
